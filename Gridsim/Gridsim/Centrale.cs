...
namespace Gridsim
{
    class Centrale
    {
        ...
        public string info()
        {
            switch (this.type)
            {
                case "Centrale éolienne":
                    return string.Format("La {0} produit {1} avec une météo : {2} pour le cout de {3}", type, Power, this.meteo.getMeteo(), curCost);
                case "Centrale à gaz":
        ...
        public void off()
        {
            this.PowerSave = this.Power;
            this.Power = 0;
        }
        public void on()
        {
            this.Power = this.PowerSave;
        }
    }
}
